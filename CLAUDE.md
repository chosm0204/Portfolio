...
| 섹션 제목·이미지·카드 | 진입 시 14px 올라오며 나타남. **한 번만** | 460ms |

- easing `cubic-bezier(.2,0,0,1)`
- **`prefers-reduced-motion` 대응 필수** — 전부 즉시 완성 상태로
- ❌ 패럴랙스 · 스크롤 하이재킹 · 프리로더 · 커스텀 커서 · 반복 재생되는 진입 효과

**진입 효과의 조건** (2026-08-31 추가) — 아래를 전부 지킬 때만 허용한다.
- 초기 상태를 CSS에 두지 않는다. **JS가 실패하면 아무것도 숨지 않아야 한다**
- 한 번만 재생. 되돌아와도 다시 움직이지 않는다
- LCP 요소에는 걸지 않는다
- 인쇄·`prefers-reduced-motion`에서 즉시 완성 상태
- 한 요소가 두 번 움직이지 않는다 (카드가 나타난 뒤 그 안의 사진이 또 움직이지 않게)
